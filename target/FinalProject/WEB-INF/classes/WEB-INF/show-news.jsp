...
<body>
<%--    <c:choose>--%>
<%--        <c:when test="${role eq admin && not empty showAllNews}">--%>
...
<%--                    <c:when test="${applicationScope.map.get(requestScope.newsList.get(loop.index).id) eq null}">--%>
<%--                <span id="${news.id}" style="position: absolute; left: 315px;">0</span>--%>
...
<%--<%=String.valueOf(request.getAttribute("a"))%>--%>